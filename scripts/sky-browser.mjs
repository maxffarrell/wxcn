...
try {
	for (const framework of ['react', 'vue']) {
		...
		for (const [label, period] of [
			['Solar noon', 'midday'],
			['Solar midnight', 'night']
		]) {
			await page.getByRole('button', { name: 'Sky time', exact: true }).click();
			await page.getByRole('menuitemradio', { name: label, exact: true }).click();
			const sky = page
				.locator(`[data-${framework}-forecast="weather"] [data-background-style="realistic"]`)
				.first();
					...
					sky
						...
						.evaluate(
							(canvas) =>
								!!canvas
									.getContext('webgl')
									?.getParameter(canvas.getContext('webgl').CURRENT_PROGRAM)
						)
				)
				.toBe(true);
			...
			const rendered = await sky.locator('canvas').evaluate((canvas) => {
				...
				return {
					...
					linked:
						...
						gl.getProgramParameter(gl.getParameter(gl.CURRENT_PROGRAM), gl.LINK_STATUS),
					width: canvas.width,
	...
	console.log(
		'Both native astronomical shaders update noon/night and render without WebGL errors.'
	);
} finally {
	await browser.close();
}
