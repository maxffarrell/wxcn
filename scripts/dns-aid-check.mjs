...
if (!record?.includes(' HTTPS 1 ')) failures.push('DNS-AID records must use ServiceMode priority 1 or greater');
if (!record?.includes('alpn=') || !record?.includes('port=')) {
	failures.push('DNS-AID records must declare alpn and port');
}
if (!record?.includes('mandatory="alpn,port"')) failures.push('alpn and port must be mandatory in the template');
if (!template.includes('<endpoint>.') || !template.includes('<alpn>') || !template.includes('<port>')) {
