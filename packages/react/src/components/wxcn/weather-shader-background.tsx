...
void main(){
 ...
 gl_FragColor=vec4(color,1.);
		...
		function draw(now = performance.now()) {
			...
			const currentSky = settings.current.sky;
			gl!.uniform1f(astronomical, currentSky ? 1 : 0);
			gl!.uniform4f(
				sunPosition,
				currentSky?.sun.x ?? 0.8,
				currentSky?.sun.y ?? 0.76,
				currentSky?.sun.altitude ?? 45,
				currentSky?.sun.visible ? 1 : 0
			);
			gl!.uniform4f(
				moonPosition,
				currentSky?.moon.x ?? 0,
				currentSky?.moon.y ?? 0,
				currentSky?.moon.altitude ?? -90,
				currentSky?.moon.visible ? 1 : 0
			);
			gl!.uniform3f(
				moonLight,
				...(currentSky?.moon.light ?? ([0, 0, -1] as [number, number, number]))
			);
	...
	.wxcn-sky[data-sky-period='sunrise'] { background: linear-gradient(180deg,#475575,#bd8290 70%,#dda472); }
	.wxcn-sky[data-sky-period='sunset'] { background: linear-gradient(180deg,#424760,#b4716b 70%,#d39866); }
