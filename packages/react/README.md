...
Props retain the Svelte meanings for `interactive`, size (`sm`, `default`, `lg`), density (`compact`, `comfortable`), detail (`simple`, `summary`, `detailed`), units, timezone, and provider data; use React `className`. Set `interactive` to enable week and day navigation; it defaults to `false`. Weather and dashboard `background` accepts `none`, `realistic`, `dithered`, or `gradient`; realistic skies animate while gradient skies remain still. Both use the location and timestamp to place the Sun and Moon and select sunrise, daytime, sunset, or night. Atmospheric motion respects reduced motion and pauses when hidden. The native icon adapter supports Lucide, Tabler, Phosphor, Hugeicons, and Remixicon using React-local icon names.
