<script lang="ts">
	...
	import * as Tabs from '@wxcn/svelte/components/ui/tabs/index.js';
...
<figure
	...
	class="relative my-4 min-w-0 overflow-hidden rounded-xl border bg-muted/30"
>
					...
					>
						{#snippet child({ props })}
							...
							<div {...rest} class={cn(className as string, (hidden as boolean) && 'hidden')}>
								<pre class="m-0! border-0! bg-transparent! p-0!"><code
			...
			<Tooltip.Trigger onclick={() => clipboard.copy(commandText)}>
				{#snippet child({ props })}
					<Button
						...
						size="icon"
						variant="ghost"
						class="absolute end-2 top-1.5 z-10 size-7 opacity-70 hover:opacity-100 focus-visible:opacity-100"
					>
	...
	<span class="sr-only" role="status"
		>{clipboard.status === 'failure'
			? 'Unable to copy. Select and copy the command manually.'
			: clipboard.copied
				? 'Copied to clipboard'
				: ''}</span
	>
