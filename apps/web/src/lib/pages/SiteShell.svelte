...
<Tooltip.Provider>
		...
		<div class="flex h-16 items-center gap-6 px-4 md:px-6">
			...
			<nav aria-label="Main navigation" class="hidden items-center gap-5 text-sm lg:flex">
				{#each mainNavItems.slice(1) as link}<a
						href={page.href(link.href)}
						class="text-muted-foreground hover:text-foreground aria-[current=page]:text-foreground"
						aria-current={page.path === link.href ? 'page' : undefined}>{link.title}</a
					>{/each}
			</nav>
			<div class="ml-auto flex items-center gap-2">
				<GithubLink />
				<div class="h-4 w-px bg-border"></div>
				<ModeSwitcher />{#if page.path !== '/'}<Button href={page.href('/')} size="sm"
						>Customize</Button
					>{/if}
			</div>
		</div>
	...
	{#if page.path !== '/'}<footer
		...
		>
			Built with <a
				href={page.framework === 'react'
					? 'https://ui.shadcn.com'
					: 'https://www.shadcn-svelte.com'}
				class="underline underline-offset-4"
				>{page.framework === 'react' ? 'shadcn/ui' : 'shadcn-svelte'}</a
