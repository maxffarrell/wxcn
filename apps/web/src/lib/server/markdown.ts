...
export const markdownPages = [
	...
	{ path: '/vue', title: 'wxcn for Vue', markdown: '/vue/index.md' },
	{ path: '/vue/docs/components', title: 'Vue components', markdown: '/vue/docs/components.md' },
	{ path: '/vue/docs/endpoints', title: 'Vue data sources', markdown: '/vue/docs/endpoints.md' }
];

export async function pageMarkdown(path: string, origin: string): Promise<string | null> {
	const framework =
		path === '/react' || path.startsWith('/react/')
			? 'react'
			: path === '/vue' || path.startsWith('/vue/')
				? 'vue'
				: 'svelte';
	const prefix = framework === 'svelte' ? '' : `/${framework}`;
	const pagePath = path.slice(prefix.length) || '/';
	const library =
		framework === 'react' ? 'shadcn/ui' : framework === 'vue' ? 'shadcn-vue' : 'shadcn-svelte';
	const installer =
		framework === 'react'
			? 'shadcn@latest'
			: framework === 'vue'
