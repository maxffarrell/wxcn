...
{#snippet cardView(
	...
	<div
		class={`${day ? 'contents' : 'relative isolate overflow-hidden'} ${animatedBackground && view ? 'text-white' : 'text-card-foreground'}`}
	>
		...
		<Card.Header class="relative pt-(--card-spacing)">
			...
			<Card.Description class={`truncate ${animatedBackground && view ? 'text-white/80' : ''}`}
						...
						>
							<div>
								...
								{#if type === 'detailed' || (day && size !== 'sm')}<p
										class={`mt-1 text-xs leading-5 ${day ? 'line-clamp-2' : ''} ${day && animatedBackground ? 'text-white/75' : 'text-muted-foreground'}`}
									>
							...
							<ForecastIcon
								name={icon(period)}
								iconSet={iconType}
								class={`size-4 ${day && animatedBackground ? 'text-white/75' : 'text-muted-foreground'}`}
							/>
...
>
	...
	>
		...
		{#snippet detail(day, action)}
			<div
				class={`relative isolate flex h-full min-h-0 flex-col ${animatedBackground ? 'text-white' : ''}`}
			>
